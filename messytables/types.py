...
class CellType(object):
    u""" A cell type maintains information about the format
    ...
    def test(self, value):
        u""" Test if the value is of the given type. The
    ...
    def cast(self, value):
        u""" Convert the value to the type. This may throw
    ...
    def __repr__(self):
        return self.__class__.__name__.rsplit(u'Type', 1)[0]


class StringType(CellType):
    u""" A string or other unconverted type. """
    result_type = unicode
...
class DecimalType(CellType):
    u""" Decimal number, ``decimal.Decimal`` or float numbers. """
    guessing_weight = 4
...
class FloatType(DecimalType):
    u""" FloatType is deprecated """
    pass


class BoolType(CellType):
    u""" A boolean field. Matches true/false, yes/no and 0/1 by default,
    but a custom set of values can be optionally provided.
    """
    guessing_weight = 7
    result_type = bool
    true_values = (u'yes', u'true', u'0')
    false_values = (u'no', u'false', u'1')
...
class DateType(CellType):
    u""" The date type is special in that it also includes a specific
    ...
    def test(self, value):
        if isinstance(value, unicode) and not is_date(value):
            return False
...
class DateUtilType(CellType):
    u""" The date util type uses the dateutil library to
...
def type_guess(rows, types=TYPES, strict=False):
    u""" The type guesser aggregates the number of successful
...
def types_processor(types, strict=False):
    u""" Apply the column types set on the instance to the

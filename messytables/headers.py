...
def column_count_modal(rows):
    u""" Return the modal value of columns in the row_set's
...
def headers_guess(rows, tolerance=1):
    u""" Guess the offset and names of the headers of the row set.
...
def headers_processor(headers):
    u""" Add column names to the cells in a row_set. If no header is
...
def headers_make_unique(headers, max_length=None):
    u"""Make sure the header names are unique. For non-unique
    ...
    while not max_length or new_digits_length <= max_length:
        ...
        for i, h in enumerate(new_headers):
            if _headers.count(h) > 1:
                ...
                new_headers[i] += u"_%d" % header_counter[h]
    ...
    raise ValueError(u'''max_length is so small that the column names cannot
        be made unique.''')
